...
Markdown 高亮由孔俊和王海涛共同完成，Markdown 预览由孔俊和徐楚佳共同完成，国际化由王海涛完成，其余工作由孔俊完成。
...
- 在主流 C++ 17 编译器及 Qt 代码分析工具 clazy 上 0 warning
...
本项目使用 C++17 开发，基于 Qt 6.0.0，但也支持 Qt5。

开发过程中发现 Qt6 各模块开发进度不同，Qt 6.0.0 没有实现 Markdown 预览依赖的 Web 模块，这个功能可能要等到 Qt 6.2.0 才能完成。因此被迫修改了部分源代码，使其兼容 Qt5，但由于使用了 C++ 17,在 Windows 平台上似乎无法编译，建议使用 Qt6。
...
- [ ] 使用中文输入法可以在 normal 模式插入字符
    这个 bug 在 Qt Creator 也存在，大概率是 Qt 的问题，很可能无法解决
