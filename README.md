...
开发过程中发现 Qt6 各模块开发进度不同，Qt 6.0.0 没有实现 Markdown 预览依赖的 Web 模块，这个功能可能要等到 Qt 6.2.0 才能完成。因此被迫修改了部分源代码，使其兼容 Qt5，但由于使用了 C++ 17,在 Windows 平台上似乎无法编译，在 Linux 平台上使用 gcc 10 成功编译。
